...
def get_dummy_uploaded_image(name=u'dummy_img.png'):
    return SimpleUploadedFile(name, get_dummy_img(), content_type='image/png')


def get_dummy_uploaded_file(name=u'dummy_file.txt'):
    return SimpleUploadedFile(name, 'HelloWorld', content_type='plain/text')


def get_dummy_uploaded_document(name=u'dummy_file.odt', size=128):
